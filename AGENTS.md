...
- **`exp` inside an iterated map.** NCA applies its rule thousands of times.
  Gain 1.05 over 100 steps is 131.5x; 0.95 is 0.0059. Use `eml_stable` only —
...
eml-nca/
  ...
  docs/                    3 files  generated catalogs and long-form notes
  experiments/             2 files  the experiment ladder, rung by rung
  harness/                 3 files  provenance, metrics contract, invariant enforcement
  journal/                 3 files  dated research log -- what was tried, what it showed
  literature/              2 files  the paper corpus and its hyperlinked index
  results/                 3 files  metrics.json per run (artifacts are NOT committed)
  ...
  src/                     7 files
    emlnca/                  7 files  EML-family primitives, stability instrumentation
  tests/                  10 files  invariants (hard-fail) -- never research metrics
  threads/                 1 file   parallel research threads: fork, run, converge
  viz/                     3 files
    manim/                   1 file   concept explainer animations
    three/                   2 files  interactive WebGL viewer
```

_68 tracked files. Generated by `scripts/gen_docs.py` -- do not edit by hand._
<!-- END:REPOMAP -->
...
| `test_lenia.py` | 22 | Tests for the Lenia simulator |
| `test_manim_explainers.py` | 10 | Tests for the Manim explainers |
| `test_security_scan.py` | 18 | Tests for the security scanner |
| `test_viz_lenia.py` | 10 | Tests for the WebGL Lenia viewer |
| **total** | **136** | all hard-failing invariants |
<!-- END:TESTCATALOG -->
